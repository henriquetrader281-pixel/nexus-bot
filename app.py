...
def renderizar_card_produto(idx, nome, valor, calor, ticket, link, mkt_alvo):
    ...
    with st.container(border=True):
        ...
        with c1:
            ...
            st.caption(f"💰 {valor} | 🎫 {ticket}") # LINHA 64 CORRIGIDA
        with c2:
            try:
...
with tabs[0]:
    ...
    if st.session_state.get("res_busca"):
        ...
        for idx, linha in enumerate(linhas):
            if "|" in linha:
                try:
                    # Fatiador robusto para não perder nome nem link
                    d = {k.split(":")[0].strip().upper(): k.split(":")[1].strip() for k in linha.replace("**", "").split("|") if ":" in k}
